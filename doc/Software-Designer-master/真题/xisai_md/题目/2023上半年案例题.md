...
- 练习页: https://wangxiao.xisaiwang.com/tiku2/exam534904225.html
- 题量: 6
...
问题一
通过“平台管理员管理租户”可以知道E1是平台管理员。
通过“系统会在设定时间向农户进行农事提醒”可以知道E2是农户。
通过“租户统一维护化肥、杀虫剂等投入品信息”可以知道E3是租户。
通过“系统也给其他第三方软件提供API接口发起农事信息请求”可以知道E4是第三方软件。
问题二
题目没有明确说D1的名称，结合P1是人员管理，D1可以命名为“人员表”。
通过下图可以知道，D2是“基地及地块数据表”。
![q01_st60309541_analysis_1](../题目素材/2023上半年案例题/Kg3s2fMf6k.png)
通过下图可以知道，D3是“农事过程表”。
![q01_st60309541_analysis_2](../题目素材/2023上半年案例题/fqFQZsZab9.png)
通过下图可以知道，D4是“农事活动表”。
![q01_st60309541_analysis_3](../题目素材/2023上半年案例题/uttc8KfPkg.png)
问题三
根据“农户在农事活动中设定投入品的实际消耗”可知，
缺少P4到D4的数据流“投入品实际消耗”。
根据P5的描述需要对农事信息进行查询，所以缺少D3到P5、D4到P5的数据流。
根据“农户根据相应农事过程提醒进行农事活动并记录”可知，缺少P3到E2的数据流。
备注：本题有较多缺失数据流，不止上面4条。
...
【问题1】（5分）
![q02_st60299925_answer_1](../题目素材/2023上半年案例题/RqYe6cGaYY.png)
注：三元联系每条边以及多重度记1分，2个属性每个1分。
【问题2】（3分）
（1）
a：零件编码   
b：采购数量
（2）
主键（车型编号，供应商名称，零件编号，采购日期）
外键（车型编号，供应商名称，零件编号）
注：关系模式补充每个1分，多写全扣；主键、外键每个0.5分，多写、少写、错写全扣。
【问题3】（7分）
（1）
![q02_st60299925_answer_2](../题目素材/2023上半年案例题/9PF3qnXzz7.png)
注：二元联系及多重度1分，实体属性1.5分，联系属性1分，少写每个属性扣除0.5分，多写、错写全扣。
（2）
门店（编号，地址，电话）；主键：门店编号；外键：无。
销售（门店编号，车型编号，销售数量，销售日期）；主键（门店编号，车型编号，销售日期）；外键（门店编号，车型编号）。
注：门店关系模式1分，主键0.5分；销售关系模式1分，主键、外键各0.5分。多写、错写、少写全扣。

### 解析

问题一
题干说补充ER图，没有明确说只需要补充联系。
通过“记录零件采购信息，某个车型的某种零件可以从多家供应商采购，某种零件也可以被多个车型采用，某家供应商也可以供应多种零件，还包括采购数量和采购日期。”可知供应商、车型、零件三者质检是*：*：*的关系，此时只拿到了3分。
仔细观察，发现“采购数量和采购日期”两者是采购的属性，一起刚好5分。
问题二
采购实体需要包含相关联三个实体的主键，所以缺少“零件编码”。
结合问题一，发现还缺少“采购数量”。
所以（a）、（b）两空就填两者，顺序可以交换。
此时的主键是三个关联实体的主键合起来，要注意书写格式。
每一个关联实体的主键又是一个外键。
问题三
题干的新需求已经明确说明新增一个实体“门店”、一个联系“销售”。
“编号、地址和电话”是“门店”的主要属性。
“销售数量和销售日期”是“销售”的主要属性。
利用如上信息可以修改ER图。
额外需要注意的是新增关系模式。
其中销售关系模式的主键由车型和门店的主键组合而成。
...
【问题1】
C1：Resource   C2：ConferencePaper  C3：JournalArticle  C4：Thesis   C5：User    C6--C8不限顺序  C6：Student   C7：Teacher   C8：Staff
【问题2】 
C1：
...
问题一
先从最显眼的C1~C4入手。C1是父类，C2~C4是子类。
根据“学术资源包括会议论文（Conference Paper）、期刑论文
（Journal Article）
以及学位
（Thesis）等”
可知，C1是Resource，C2到C4是Conference Paper 、Journal Article 、Thesis 。
C2和会议集
（Proceeding）
有组合关系，所以C2是会议论文Conference Paper。
C3和期刊
（Edition）
有组合关系，所以C3是期刊论文Journal Article。
C5是父类，C6~C8是其子类，所以C5是User用户，C6到C8是Student学生、Teacher老师、Staff员工，三者顺序可以交换。
问题二
C1是C2~C4的父类，拥有公共属性，子类可以有特有的属性。
根据题干可知：
...
问题三
问题明确说明一旦数据（引用次数）变化，要及时通知相关用户。
这和观察者模式的应用一致。观察者模式定义对象间的一对多的关系，当一个对象的状态发生改变，所有依赖于它的对象都将得到通知并自动更新。
...
（5）return count  
【问题2】
![q04_st60386379_answer_1](../题目素材/2023上半年案例题/s3fCp3b5RT.png)

### 解析

第一空是判断顶点p-  > adjvex是否被访问。
第二空是访问顶点的下一个节点。
第三、四空是用count计算连通分量数。count应该赋值为0。
第五空返回count的结果即可
